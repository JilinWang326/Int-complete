...
/-!
# Finite prefixes for Veldman's construction

This file formalizes the finite sequences `a ∈ Seq` from paper §3.1 and the
prefixes `α↾n` of infinite branches `α`.  It is Lean infrastructure for the
recursive definitions of `Σ`, `Γ`, and subfans in §3 and §4.
-/

/-- The type of finite sequences of natural numbers, i.e. the paper's `Seq` from §3.1. -/
  ...
  ⟨a.len + b.len, fun i =>
    if h : i.val < a.len then
      ...
      b.seq ⟨i.val - a.len, by
        ...
        have hi : i.val < b.len + a.len := by
          simp [Nat.add_comm]
        exact Nat.sub_lt_right_of_lt_add ha hi
      ⟩⟩
