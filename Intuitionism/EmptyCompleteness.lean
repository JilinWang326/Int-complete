...
import Intuitionism.ImplicationSubfan
import Intuitionism.BarInduction

/-!
# Final assembly of the concrete completeness proof

This file packages the concrete fan, the sGammaRuless proof, the implication-case
...
open ConcreteGammaRules ImplicationSubfan
...
/-- Instantiate the abstract completeness context with the concrete Veldman fan. The fields correspond respectively to semiregularity of sΓ_αs, the implication hard case (paper §4.1), and the local bar-induction step used to push derivability back to the root. -/
noncomputable def ctxConcrete
    (hBar : BarInductionStd) (E : Enumerations) :
    ImpHardData.CompletenessCtx (ImplicationSubfan.Vconcrete E) := by
  refine
  { hR := ?_
    BarIndStd := hBar
    impData := ImplicationSubfan.impDataConcrete (E := E)
    barIndStep := ?_ }

  ·
    simpa [ImplicationSubfan.Vconcrete, ImplicationSubfan.toConcreteEnum,
      ConcreteGammaRules.Concrete.mkConcreteFan, ConcreteGammaRules.Concrete.Enumerations.toConcrete] using
      (ConcreteGammaRules.Main.gammaRules_concrete (E0 := E))

  ·
    intro A s hs0 hall
    let E0 : IPC.VeldmanConcrete.Enumerations := ImplicationSubfan.toConcreteEnum E

    have hs0' : IPC.VeldmanConcrete.Sigma E0 s = 0 := by
      simpa [ImplicationSubfan.Vconcrete, E0] using hs0
    ...
    have hall' :
      ...
      intro n hn0
      have hn0V : (ImplicationSubfan.Vconcrete E).S (extend s (singleton n)) = 0 := by
        simpa [ImplicationSubfan.Vconcrete, E0] using hn0
      have hprf :
          ((↑((ImplicationSubfan.Vconcrete E).F (extend s (singleton n))) : Set Form) ⊢ᵢ A) :=
        hall n hn0V
      simpa [ImplicationSubfan.Vconcrete, E0] using hprf

    simpa [ImplicationSubfan.Vconcrete, E0] using
